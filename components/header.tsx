...
export default function Header() {
  ...
  // Client-side only: get query param from URL
  useEffect(() => {
    ...
    <>
      ...
      <header className="hidden md:block bg-header-leather text-white fixed top-0 w-full z-50 shadow-sm">
        <div className="max-w-7xl container-max px-6 py-6 flex justify-between items-center">
          <Link href="/" className="text-2xl font-serif font-light tracking-widest text-white">
            FlexLeather
          </Link>
          ...
          <div className="flex-1 mx-8 flex items-center">
            <div className="w-full flex items-center border border-border bg-white/5 px-3 py-2 rounded-md backdrop-blur-sm shadow-sm">
              <Search
                className="w-4 h-4 text-white/80 cursor-pointer"
                onClick={() => {
                  if (search) window.location.href = `/shop?q=${encodeURIComponent(search)}`
                }}
              />
              <input
                type="text"
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && search.trim()) {
                    window.location.href = `/shop?q=${encodeURIComponent(search.trim())}`
                  }
                }}
              />
            </div>
        ...
        <div
          ...
          style={{
            ...
            height: '12px',
            pointerEvents: 'none',
            background: 'linear-gradient(90deg, rgba(0,0,0,0.18), rgba(0,0,0,0.06), rgba(0,0,0,0.18))',
            filter: 'blur(6px)',
          }}
        />
      </header>
      ...
      <header className="md:hidden bg-header-leather text-white fixed top-0 w-full z-50 shadow-sm">
        <div className="px-4 py-4 flex justify-between items-center">
          <Link href="/" className="text-xl font-serif font-light tracking-widest text-white">
            FlexLeather
          </Link>
          <div className="flex gap-4 items-center">
            <Link href="/cart" className="relative">
              <ShoppingCart className="w-5 h-5 text-white" />
              {totalItems > 0 && (
                <span className="absolute -top-2 -right-2 bg-accent text-accent-foreground text-xs rounded-full w-4 h-4 flex items-center justify-center">
                  {totalItems}
                </span>
              )}
            </Link>
